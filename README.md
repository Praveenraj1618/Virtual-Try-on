...
## V6 garment outfits and cut preservation

The workspace holds up to four independently editable garments. Add keeps existing pieces; Replace edits the selected one. The catalogue includes short-sleeve tees, V-neck blouses, flared tunics, A-line/straight/maxi dresses, and straight/wide/tapered trousers. These are distinct procedural cuts, not an arbitrary garment reconstruction system. Top + trousers is the supported layering baseline. Each garment has its own geometry, colour, fabric, print and dimensions. One mannequin is rendered for the entire outfit. A simple trousers clearance envelope supports untucked upper pieces; overlapping upper layers are approximate and do not use cloth-to-cloth simulation.

Outfit layers are stored inside the existing design JSON; existing single-garment looks still load. All image references in every layer are checked against the owner before saving. No database migration is required.

Pencil detection smooths paper grain and joins nearby disconnected strokes instead of discarding faint skirt outlines. A narrowing beneath a wide hem suggests a separate lower piece; users confirm it with the trousers option. Waist landmarks follow the narrower upper contour, and traced waist placement and hem circumference are retained by the geometry. Cap/short/long sleeve overrides and bottom-edge circumference controls allow corrections. Exact neckline panels, crossed construction, bows, embroidery, cut-outs and sewing patterns are not inferred.

The shoulder regression is addressed by keeping bodice vertices torso-bound and limiting arm influence to the sleeve, with graduated influence at the armhole. The centre chest no longer inherits shoulder rotation. Cloth remains a visual approximation rather than a calibrated fabric solver.
